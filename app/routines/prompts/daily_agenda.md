...
- **The read-but-unanswered check:** your runtime context carries **"Threads awaiting a Boxx reply"**, computed from the local 90-day mail index — read mail included, so it catches what Arda opened and forgot (a filming inquiry, a hanging customer question). Triage that list instead of searching: open the promising ones with `get_gmail_message` (the list gives the message id), surface what needs Arda, and task it. If the section is absent, the index isn't built yet — fall back to `in:inbox newer_than:14d` header scans on both mailboxes and judge from headers which threads got no Boxx reply.
